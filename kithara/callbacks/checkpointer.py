...
class Checkpointer(Callback):
    ...
    def __init__(
        ...
        assert (self.by_batch and not self.by_epoch) or (
            self.by_epoch and not self.by_batch
        ), "One and only one of `by_batch` and `by_epoch` should be True."
        ...
        ), "Please provide the model instance when creating the Checkpointer."
    ...
    def save(self, step, blocking=False):
        ...
        state = {v.path: v.value for v in self.model.variables}
        ...
        if self.mngr.reached_preemption(step):
            ...
            self.mngr.wait_until_finished()
            print(
                f"✅ Successfully saved checkpoint to {os.path.join(self.checkpoint_dir, str(step))}"
            )
            exit()
    ...
    def load(self, step=None, in_place=True):
        ...
        state = self.mngr.restore(
            step, args=ocp.args.StandardRestore(jax.tree.map(set_dtype, abstract_state))
        )
